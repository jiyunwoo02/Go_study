...
func main() {
	...
	// 메인 고루틴에서 `range`를 사용하여 채널에서 데이터를 수신 + 동시에 고루틴이 채널에 데이터를 전송
	for val := range ch { // range 문은 채널이 닫힐 때까지 반복하여 채널에서 데이터를 읽는다
		// 채널이 닫힌 상태에서 버퍼에 데이터가 모두 소모되면, range 문은 더 이상 데이터가 없기 때문에 종료
	...
	*/
	...
	// 메인 고루틴의 for 루프는 채널에 데이터가 들어오면 데이터를 수신하여 출력하며,
	// 고루틴은 채널에 데이터를 보내고 채널을 닫는 작업을 진행

	// 2. 슬라이스와 Range
	// : 슬라이스의 첫 번째 요소부터 마지막 요소까지 순차적으로 접근
	fmt.Println("\n2. 슬라이스 예제")
	slice := []int{10, 20, 30, 40} // 정수형 슬라이스 생성
	...
	*/
	...
	// 슬라이스는 요소들이 순서대로 저장되며, 각 요소는 0부터 시작하는 정수형 인덱스를 가진다.
	// => 따라서 슬라이스를 순회하면 요소들이 항상 삽입된 순서대로 반환

	// 3. 맵과 Range
	// : 맵의 모든 키-값 쌍을 확인할 때까지 순회
	fmt.Println("\n3. 맵 예제")
	m := map[string]int{"a": 0, "b": 200, "c": 300} // 문자열 키와 정수 값을 가지는 맵을 생성
	// `range`를 사용하여 맵을 반복, 각 키와 값을 출력
	// 다만, 맵은 순회할 때 요소가 무작위 순서로 반환될 수 있다.
	...
	*/
	...
	// 맵은 키와 값의 쌍으로만 동작하기 때문에 인덱스를 사용해서 접근 X, 입력 순서 보장 X
	// 인덱스가 없고, 키에 의해 접근되기 때문에, 순회 시 요소들이 특정한 순서 없이 반환
}
...
[2] 채널이 닫힐 때: 채널이 닫히면, range 문은 더 이상 데이터를 수신하지 않고 루프를 종료
...
# 고루틴이 채널을 닫은 후에도,
for 루프는 정상적으로 작동하며 채널이 닫힐 때까지 데이터를 수신
*/
