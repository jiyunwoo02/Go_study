...
func main() {
	...
	ctx, cancel := context.WithCancel(context.Background()) // 취소 가능한 컨텍스트 ctx와 취소 함수 cancel 생성
	...
	wg.Wait() // WaitGroup의 카운터가 0이 될 때까지 대기 -> 모든 고루틴이 종료될 때까지 기다림
}

// PrintSecond 함수: 매 초마다 "Tick"을 출력하는 고루틴
func PrintSecond(ctx context.Context) {
	tick := time.Tick(time.Second) // 1초마다 신호를 보내는 채널 생성 -> 1초마다 출력하는데, 카운터는 변하지 X
	for {
		select {
		case <-ctx.Done(): // 컨텍스트의 취소 신호를 받으면 고루틴 종료
			wg.Done() // WaitGroup의 카운터를 감소(-1)시켜 메인 고루틴이 Wait() 대기를 마치게 함 -> 프로그램 종료
			return
