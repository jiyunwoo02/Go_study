...
func main() {
	...
	go func() {
		time.Sleep(2 * time.Second) // 2초 대기 (즉시 전송하지 않고 2초 후에 전송)
		ch1 <- "ch1에서 수신된 메시지"      // ch1에 문자열 데이터를 전송
	}()

	// 3. 두 번째 고루틴: ch2에 데이터를 전송하는 송신자
	go func() {
		time.Sleep(1 * time.Second) // 1초 대기 (1초 후에 전송)
		ch2 <- "ch2에서 수신된 메시지"      // ch2에 문자열 데이터를 전송
	}()

	// 3-1. 메인 고루틴의 대기
	time.Sleep(5 * time.Second) // 고루틴이 데이터를 채널에 전송할 시간을 준다

	// 4. 메인 고루틴에서 select 문을 사용하여 채널 데이터를 수신
	// 메인 고루틴: 수신자
	...
	// -> 메인 고루틴이 3초 대기하는 동안에 두 고루틴은 송신하려고 대기하고 있다가, 메인 고루틴이 select 문을 실행함으로써 데이터를 송신

	// 프로그램 종료 전 대기: 2초 동안 대기하여 고루틴이 종료될 수 있는 시간을 확보
