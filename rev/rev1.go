...
func swap(x, y int) (int, int) { // (3) 1과 2는 함수의 파라미터로 전달되면서 x와 y라는 변수에 복사되어 저장된다
	// x와 y의 메모리 주소 출력
	fmt.Printf("swap 함수의 x의 메모리 주소: %p\n", &x)
	fmt.Printf("swap 함수의 y의 메모리 주소: %p\n\n", &y)
	return y, x // 두 개의 값을 반환
}

func main() {
	fmt.Println("Tuple assignment에 대해 알아보자\n")
	a, b := swap(1, 2)                                  // (1) swap(1, 2)가 호출되면, 함수 내부에서 1과 2가 각각 x와 y로 전달된다. -> (2) x에는 1이, y에는 2가 할당된다 -> (4) y는 2로, x는 1로 평가되어 임시 저장소에 저장된다.
	fmt.Printf("swap 함수의 결과 -> a : %d, b : %d\n", a, b) // (5) 좌측의 a와 b는 새로 선언되는 변수이므로, 각각 메모리 공간이 할당된다. -> (6) 임시 저장소에 있는 값들이 좌측 변수 a와 b에 복사된다.
	// 출력: 2 1
	...
	fmt.Printf("y1의 메모리 주소: %p\n", &y1)   // y1의 메모리 주소 출력
	fmt.Printf("z1의 메모리 주소: %p\n\n", &z1) // z1의 메모리 주소 출력

	// 3. 기존 변수와 새 변수에 값 할당 및 업데이트
	// 기존에 선언된 변수와 새로 선언되는 변수에 동시에 값을 할당
	x2 := 1
	y2 := 2
	x2, z2 := 3, 4                                       // x2는 기존 변수, z2는 새 변수 -> 이 경우, x2는 기존에 선언된 변수로 값을 업데이트하고, z2는 새 변수로 할당
...
Go 런타임이 이 반환값을 처리할 때, 각각의 반환값이 새로운 메모리 공간에 저장된다
...
각 값은 별도의 메모리 주소에 저장되며, 여러 값을 동시에 처리할 수 있는 강력한 기능을 제공합니다.
*/
